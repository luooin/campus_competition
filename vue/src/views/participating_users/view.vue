		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="用户学号" prop="user_student_id">
												<el-input id="user_student_id" v-model="form_sub['user_student_id']" placeholder="请输入用户学号"
							  v-if="user_group === '管理员' || (form_sub['participating_users_id'] && $check_field('set','user_student_id')) || (!form_sub['participating_users_id'] && $check_field('add','user_student_id'))"></el-input>
					<div v-else-if="$check_field('get','user_student_id')">{{form_sub['user_student_id']}}</div>
											</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','user_name') || $check_field('add','user_name') || $check_field('set','user_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="用户姓名" prop="user_name">
												<el-input id="user_name" v-model="form_sub['user_name']" placeholder="请输入用户姓名"
							  v-if="user_group === '管理员' || (form_sub['participating_users_id'] && $check_field('set','user_name')) || (!form_sub['participating_users_id'] && $check_field('add','user_name'))"></el-input>
				...
				<el-form-item label="用户性别" prop="user_gender">
						...
						v-if="user_group === '管理员' || (form_sub['participating_users_id'] && $check_field('set','user_gender')) || (!form_sub['participating_users_id'] && $check_field('add','user_gender'))">
				...
				<el-form-item label="用户年龄" prop="user_age">
							  ...
							  v-if="user_group === '管理员' || (form_sub['participating_users_id'] && $check_field('set','user_age')) || (!form_sub['participating_users_id'] && $check_field('add','user_age'))"></el-input>
	...
	export default {
		...
		data() {
			return {
				...
				group_table: "participating_users",
				is_password: true,
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["participating_users_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?participating_users_id=" + form_sub["participating_users_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/participating_users/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/participating_users/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/participating_users/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/participating_users/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/participating_users/view','get');
