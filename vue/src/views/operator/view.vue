			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="状态" prop="state">
					<el-select v-model="form.state" placeholder="请选择">
						...
						<el-col v-if="user_group === '管理员' || $check_field('get','name_of_personnel') || $check_field('add','name_of_personnel') || $check_field('set','name_of_personnel')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="人员姓名" prop="name_of_personnel">
												<el-input id="name_of_personnel" v-model="form_sub['name_of_personnel']" placeholder="请输入人员姓名"
							  v-if="user_group === '管理员' || (form_sub['operator_id'] && $check_field('set','name_of_personnel')) || (!form_sub['operator_id'] && $check_field('add','name_of_personnel'))"></el-input>
					<div v-else-if="$check_field('get','name_of_personnel')">{{form_sub['name_of_personnel']}}</div>
											</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','gender_of_personnel') || $check_field('add','gender_of_personnel') || $check_field('set','gender_of_personnel')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="人员性别" prop="gender_of_personnel">
								<el-select id="gender_of_personnel" v-model="form_sub['gender_of_personnel']"
						v-if="user_group === '管理员' || (form_sub['operator_id'] && $check_field('set','gender_of_personnel')) || (!form_sub['operator_id'] && $check_field('add','gender_of_personnel'))">
						<el-option v-for="o in list_gender_of_personnel" :key="o" :label="o" :value="o">
						</el-option>
					</el-select>
					<div v-else-if="$check_field('get','gender_of_personnel')">{{form_sub['gender_of_personnel']}}</div>
							</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','age_of_personnel') || $check_field('add','age_of_personnel') || $check_field('set','age_of_personnel')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="人员年龄" prop="age_of_personnel">
												<el-input id="age_of_personnel" v-model="form_sub['age_of_personnel']" placeholder="请输入人员年龄"
							  v-if="user_group === '管理员' || (form_sub['operator_id'] && $check_field('set','age_of_personnel')) || (!form_sub['operator_id'] && $check_field('add','age_of_personnel'))"></el-input>
					<div v-else-if="$check_field('get','age_of_personnel')">{{form_sub['age_of_personnel']}}</div>
											</el-form-item>
			</el-col>
	...
	export default {
		...
		data() {
			return {
				...
				disabledObj:{
								"name_of_personnel_isDisabled": false,
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["operator_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?operator_id=" + form_sub["operator_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/operator/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/operator/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/operator/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/operator/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/operator/view','get');
